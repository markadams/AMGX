...
class MISSelectorBase : public Selector<T_Config>
{
    ...
    protected:
        ...
        int m_mis_k;              // MIS distance parameter (1=standard, 2=aggressive, 3=legal but not recommended)
        int m_aggressive_levels;  // Number of levels to use mis_k (0=no aggressive i.e. MIS-1 everywhere, N=first N levels)
        ...
        double m_mis_strength_threshold; // edges with weight < threshold*max_row_weight are ignored
        int m_verbose;               // Print detailed aggregation diagnostics (0=quiet, 1=verbose)
};
