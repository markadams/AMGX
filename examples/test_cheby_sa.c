...
// test_cheby_sa.c
//
// Test driver: standalone SA-AMG (Richardson outer) with Chebyshev(1)+Jacobi smoother.
// V(1,1) cycle: 1 Chebyshev pre-sweep, 1 Chebyshev post-sweep.
//
// Compares against PETSc standalone GAMG with Chebyshev(1)+Jacobi smoother:
//   -ksp_type richardson -ksp_richardson_scale 1.0 -ksp_norm_type unpreconditioned
...
//
// IMPORTANT: PETSc must use -ksp_norm_type unpreconditioned to match AMGx's
// convergence criterion (unpreconditioned residual norm).  Without this flag,
// PETSc Richardson uses preconditioned norm and converges in fewer iterations.
//
...
//     ../examples/test_cheby_sa.c -o test_cheby_sa \
...
//
// Run:
//   AMGX_IMPORT_AGGREGATES=gamg_aggs.txt ./test_cheby_sa poisson2d.mtx
...
// -----------------------------------------------------------------------
// Standalone SA-AMG with Chebyshev(1)+Jacobi smoother.
//
// Outer solver: AMG (Richardson/V-cycle iteration)
...
//     Matches PETSc GAMG's default Chebyshev esteig: [0, 0.1, 0, 1.1] applied
//     to rho gives emin = 0.1*rho, emax = 1.1*rho.
//   - BLOCK_JACOBI with relaxation_factor=1.0: pure D^{-1} scaling,
//     matches PETSc -mg_levels_pc_type jacobi.
//
// V(1,1) cycle: presweeps=1, postsweeps=1
// Coarse solver: Dense LU
// 2 levels max
// -----------------------------------------------------------------------
static const char *CHEBY_SA_AMG_CONFIG =
    "{"
    "  \"config_version\": 2,"
    "  \"solver\": {"
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        ...
        fprintf(stderr, "\n");
        fprintf(stderr, "Standalone SA-AMG with Chebyshev(1)+Jacobi smoother, V(1,1) cycle.\n");
        fprintf(stderr, "\n");
    ...
    printf("=== AMGx Chebyshev(1)+Jacobi SA-AMG Test ===\n");
    printf("Matrix file: %s\n", matrix_file);
    printf("Config: Standalone SA-AMG, Chebyshev(1)+Jacobi smoother, V(1,1) cycle\n");
    if (import_file)
    ...
    AMGX_config_create(&cfg, CHEBY_SA_AMG_CONFIG);
