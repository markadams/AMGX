...
//     ../examples/test_multilevel_jacobi_l1.c -o test_multilevel_jacobi_l1 \
...
//
// Run:
//   ./test_multilevel_jacobi_l1 poisson2d_400.mtx
...
// -----------------------------------------------------------------------
// PCG outer solver + SA-AMG preconditioner with Jacobi-L1 smoother.
// Multi-level: max_levels=20, min_coarse_rows=10
// -----------------------------------------------------------------------
static const char *PCG_JACOBI_L1_MULTILEVEL_CONFIG =
    "{"
    "  \"config_version\": 2,"
    "  \"solver\": {"
    "    \"solver\": \"PCG\","
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        ...
        fprintf(stderr, "\n");
        fprintf(stderr, "PCG + SA-AMG with Jacobi-L1 smoother, multi-level V(1,1) cycle.\n");
    ...
    printf("=== AMGx Multi-Level PCG + Jacobi-L1 SA-AMG Test ===\n");
    printf("Matrix file: %s\n", matrix_file);
    printf("Config: PCG outer, SA-AMG preconditioner\n");
    printf("  Smoother: Jacobi-L1, relaxation_factor=1.0, V(1,1) cycle\n");
    ...
    AMGX_config_create(&cfg, PCG_JACOBI_L1_MULTILEVEL_CONFIG);
