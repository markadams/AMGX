...
// test_mg_diag.c
//
// Minimal test driver for MG-DIAG comparison with PETSc GAMG.
// Runs ONLY the Jacobi-L1 Richardson config (no Chebyshev, no PCG runs).
// This ensures the MG-DIAG static counter in fixed_cycle.cu starts at 0
// and we get diagnostic output for the first 5 V-cycle iterations.
//
// Setup:
//   - 2-level V-cycle: fine + coarse LU
//   - Smoother: undamped Jacobi-L1 (relaxation_factor=1.0)
//   - Outer solver: Richardson (standalone AMG iteration)
//   - Aggregates: imported from GAMG via AMGX_IMPORT_AGGREGATES env var
//   - Near-null space: constant vector (1 DOF per node)
//
...
//     ../examples/test_mg_diag.c -o test_mg_diag \
...
// -----------------------------------------------------------------------
// Jacobi-L1 Richardson: undamped, 2-level, matches PETSc exactly.
//
// x_{k+1} = x_k + D_L1^{-1} (b - A x_k)
// (D_L1)_ii = sum_j |A_ij|   (L1 row norm)
// omega = 1 (undamped)
// 1 pre-sweep + 1 post-sweep
// -----------------------------------------------------------------------
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        ...
        fprintf(stderr, "\n");
        fprintf(stderr, "MG-DIAG test: runs AMGx SA with Jacobi-L1 Richardson smoother.\n");
        fprintf(stderr, "Prints [MG-DIAG] norms at each V-cycle stage for comparison with PETSc.\n");
        fprintf(stderr, "\n");
    ...
    printf("=== AMGx MG-DIAG Test ===\n");
    printf("Matrix file: %s\n", matrix_file);
    printf("Config: Jacobi-L1 Richardson, 2-level, undamped (omega=1)\n");
    if (import_file)
    ...
    AMGX_config_create(&cfg, JACOBI_L1_2LEVEL_CONFIG);
