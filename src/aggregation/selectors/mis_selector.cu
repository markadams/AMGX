    ...
    while (tid < num_rows)
    {
        if (status[tid] == MIS_UNDECIDED)
        {
            ...
            for (int j = jmin; j < jmax; j++)
            {
                ...
                // Skip weak edges: filter edges ≤ threshold (GAMG convention).
                // A negative threshold keeps all edges (all weights are positive).
                if (edge_weights[j] <= strength_threshold)
                { continue; }
            ...
            for (int j = jmin; j < jmax && !dominated; j++)
            {
                ...
                if (jcol < total_rows)
                {
                    ...
                    for (int k = kmin; k < kmax && !dominated; k++)
                    {
                        ...
                        // Skip weak edges at distance-2: filter ≤ threshold (GAMG convention).
                        if (edge_weights[k] <= strength_threshold)
                        { continue; }
    ...
    while (tid < num_rows)
    {
        ...
        else // MIS_NOT_ROOT — find strongest MIS_ROOT neighbor
        {
            ...
            for (int j = jmin; j < jmax; j++)
            {
                ...
                // Skip weak edges during assignment: filter ≤ threshold (GAMG convention).
                if (edge_weights[j] <= strength_threshold)
                { continue; }
...
MISSelectorBase<T_Config>::MISSelectorBase(AMG_Config &cfg, const std::string &cfg_scope)
{
    ...
    m_mis_strength_threshold = cfg.AMG_Config::template getParameter<double>("mis_strength_threshold", cfg_scope);
    ...
    int &num_aggregates)
{
    ...
    if (this->m_mis2_algorithm == 1 && effective_k == 2)
    {
        ...
        float strength_thresh = (float)this->m_mis_strength_threshold;
