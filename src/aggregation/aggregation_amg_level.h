...
//    typedef typename TraitsFromMatrix<Matrix>::Traits MatrixTraits;
        ...
        // Accessor for the SA spectral radius rho(D^{-1}A) estimated during setup.
        // Returns 0.0 if smoothProlongator() has not been called yet.
        double getSARho() const { return m_sa_rho; }

        // Accessors for the near-null space vectors (for testing / diagnostics).
        // m_near_null_space is device-side, column-major, size = num_rows * null_dim.
        const VVector &getNearNullSpace() const { return m_near_null_space; }
        const VVector &getCoarseNearNullSpace() const { return m_coarse_near_null_space; }
