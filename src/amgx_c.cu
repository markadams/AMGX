...
#undef AMGX_CASE_LINE
        ...
        const double *coords)
    {
        ...
        AMGX_TRIES()
        {
            ...
            // Global MGS orthonormalization (matches PETSc MatNullSpaceCreateRigidBody).
            // Uses MPI_Allreduce for global dot products so all ranks produce
            // identical, globally-orthonormal null vectors.
            amgx::aggregation::orthonormalizeRigidBodyModes(
                dim, null_dim, num_rows, B.data()
#ifdef AMGX_WITH_MPI
                , resources->getMpiComm()
#endif
            );
