...
5. **cuDSS coarse solver** *(experimental — not yet tested)* — GPU-native sparse direct solver (LU/Cholesky) as an exact coarse-grid solver, intended to replace the CPU-bound `DENSE_LU_SOLVER`
...
### cuDSS Coarse Solver *(experimental — not yet tested)*

> **Status:** Code scaffolding is in place but the cuDSS solver has **not been tested or validated**. Do not rely on it for production use.

`CUDSS_SOLVER` is intended to be a GPU-native sparse direct solver that replaces `DENSE_LU_SOLVER` as the exact coarse-grid solver in SA-AMG. It will use NVIDIA's cuDSS library (available in CUDA 12.4+) to perform sparse LU or Cholesky factorization entirely on the GPU, avoiding the CPU round-trip that `DENSE_LU_SOLVER` requires.

**Planned advantages over `DENSE_LU_SOLVER`:**
...
| `examples/bench_cudss_vs_dense_lu.c` | Performance comparison: `CUDSS_SOLVER` vs `DENSE_LU_SOLVER` on 2D Poisson *(not yet tested)* |
...
- **Multi-GPU (MPI) support** *(experimental — not yet tested)*: Initial code scaffolding for multi-GPU is in place (the MIS-k selector includes halo exchange calls and the cuDSS coarse solver includes a multi-GPU gather/scatter path), but **none of this has been tested or validated** in multi-GPU configurations. Full MPI testing on Perlmutter is a primary remaining work item.
- **cuDSS coarse solver validation:** The cuDSS solver code exists but has not been tested. See the cuDSS section above for details.
